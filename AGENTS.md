...
The animated curtains (sixteen dye colors, `<color>_curtain` blocks under `BlockRegistry.CURTAINS`) render through per-bone model files bound by `CurtainBlockEntityRenderer` to a shared animation rig — the rig/controller/clip resources under `sap/animations` and `neoforge/animations` are color-agnostic, so a color needs no new animation files. The white masters (`curtain_{upper,lower}_{r,l}.json` aggregate models, `white_curtain.json` item display model, `white.png` texture) are the only hand-authored sources. `tools/curtain/split_curtain_model.js` derives every other color as parent-reference variants (texture override only) and splits all colors into per-bone files under `models/block/curtain/curtain_<half>_<side>_<color>[_<color>]/`. After any texture/UV/geometry edit to a white master, re-run `node tools/curtain/split_curtain_model.js`, then `./gradlew.bat runClientData`, or the placed block keeps showing old assets. The renderer resolves per-bone models through `BlockModelRegistry.CURTAIN_*` sets keyed by `(DyeColor, bone)`.
