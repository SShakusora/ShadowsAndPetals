...
public class HammerClientExtensions implements IClientItemExtensions {
    ...
    static void applyThirdPersonHammerPose(HumanoidModel<?> model, LivingEntity state, HumanoidArm arm) {
        float nowSeconds = state.tickCount / TICKS_PER_SECOND;
        var playback = UseAnimationPlaybackManager.INSTANCE.find(
                state.getId(), SAPAnimations.HAMMER, nowSeconds);
        if (playback == null) {
            return;
        }
