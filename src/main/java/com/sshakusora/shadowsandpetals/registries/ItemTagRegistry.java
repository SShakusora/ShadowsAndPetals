...
public final class ItemTagRegistry {
     ...
     * The deferred entry is retained until the item tag provider runs, avoiding early registry resolution.
     */
    public static void add(TagKey<Item> tag, DeferredItem<? extends Item> item) {
        TAG_MAP.computeIfAbsent(tag, key -> new ArrayList<>()).add(item);
    }
    ...
    /**
     * Clears collected declarations for isolated data-generation/test runs.
     */
