...
The large curtains (sixteen dye colors, `<color>_large_curtain` blocks under `BlockRegistry.LARGE_CURTAINS`, a 2x2 structure with HALF/COLUMN properties) follow the same static-plus-ANIMATING architecture through `LargeCurtainBlock` and `LargeCurtainBlockEntityRenderer`. Their only hand-authored sources live in `models/block/large_curtain/` (`large_curtain_right.bbmodel` + the Blockbench geometry export `large_curtain_right.json` + `large_curtain.animation.json`); note that the animation file is Blockbench’s native Bedrock export, so X position and Y rotation channels are mirrored relative to the editor model. `tools/curtain/split_large_curtain_model.js` converts that into everything else: rig/controller/clip resources under `sap/animations/large_curtain` and `neoforge/animations/entity/large_curtain`, baked closed/open static masters per (row, column, side), sixteen-color parent-reference stubs, and per-bone parent models plus colored texture-override stubs. After any geometry or animation edit, re-run `node tools/curtain/split_large_curtain_model.js`, then `./gradlew.bat runClientData`.
